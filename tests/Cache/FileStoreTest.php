...
use Fast\Cache\FileStore;
use PHPUnit\Framework\TestCase;
use Fast\Contracts\Filesystem\FileNotFoundException;
...
class FileStoreTest extends TestCase
{
    ...
    public function testExpiredItemsReturnNull()
    {
        ...
        $store = $this->getMockBuilder('Fast\Cache\FileStore')->setMethods(['forget'])->setConstructorArgs([$files, __DIR__])->getMock();
    ...
    {
        $files = $this->mockFilesystem();
        $store = $this->getMockBuilder('Fast\Cache\FileStore')->setMethods(['expiration'])->setConstructorArgs([$files, __DIR__])->getMock();
    ...
    public function testFileSystemAndDirectoryAndPrefix() {
        $files = $this->mockFilesystem();
        $store = new FileStore($files, __DIR__.'--wrong');
        $this->assertEquals($files, $store->getFilesystem());
        $this->assertEquals(__DIR__.'--wrong', $store->getDirectory());
    ...
    {
        return $this->createMock('\Fast\Filesystem\Filesystem');
    }
}
